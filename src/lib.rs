...
pub fn process<R: Read>(
    ...
    while let Ok(orig_len) = reader.by_ref().take(max_line_length).read_line(&mut line) {
        ...
        println!("TOL: state={state:?} line_number={line_number} orig_len={orig_len} line=\"{line}\"");

        match state {
            State::BeginningOfLine => {
                if line.ends_with("\n") {
                    println!("BeginningOfLine {line_number}: state = ProcessBeginningOfLine");
                    ...
                    beginning_of_line = line.clone();
                    println!("BeginningOfLine {line_number}: state = DiscardUntilLF");
            ...
            State::DiscardUntilLF => {
                if line.ends_with("\n") {
                    ...
                    state = State::ProcessBeginningOfLine;
                    println!("DiscardUntilLF {line_number}: LF, done discarding");
                } else if orig_len == 0 {
                    state = State::ProcessBeginningOfLine;
                    println!("DiscardUntilLF {line_number}: EOF, done");
                } else {
                    full_line_len += orig_len;
                    println!("DiscardUntilLF {line_number}: no LF or EOF, continue to discard");
                }
            }
        ...
        if state == State::ProcessBeginningOfLine {
            ...
            println!("ProcessBeginningOfLine {line_number}: line_len={line_len}");
